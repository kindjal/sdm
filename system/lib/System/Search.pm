...
#Don't "use System;" here or we introduce a circular dependency.
use UR;
use System::Memcache;
...
sub _delete_by_doc {
    ...
    my $memcached = System::Memcache->server;
...
sub generate_result_xml {
    ...
    for my $doc (@docs) {
        ...
        require System; #It is only at this point that we actually need to load other objects
        ...
        if($views{$object->class}) {
            ...
            if(my $view_class = $class->_resolve_result_xml_view($object_class)) {
                ...
                if($format eq 'xsl' or $format eq 'html') {
                    $view_args{xsl_root} = System->base_dir . '/xsl';
                }
        ...
        if(defined $views{$object->class}) {
            ...
            $view->_update_view_from_subject();
             ...
             if(my $view_class = $class->_resolve_solr_xml_view($object)) {
                 ...
                 Carp::confess('To make an object searchable create an appropriate ::View::Solr::Xml that inherits from System::View::Solr::Xml.');
             }
        }
...
my %commit_cb = ();
my %delete_cb = ();
#This should be called from System.pm, so typically it won't need to be called elsewhere.
...
=head1 SYNOPSIS
  ...
  System::Search->add(@objects);
  System::Search->delete(@objects);
  System::Search->is_indexable($object);

  System::Search->search($query, $options);
...
To make a new type of object indexable by Solr, create a subclass of System::View::Solr::Xml for
that object.  For example, to make a System::Individual indexable, the class
System::Individual::View::Solr::Xml was created.  The class should define the
