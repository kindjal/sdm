...
BEGIN {
    if (my $config = $ENV{SYSTEM_CONFIG}) {
        ...
        while (@hwords) {
            my $pkg = 'System::Site::' . join("::",@hwords);
            local $SIG{__DIE__};
            local $SIG{__WARN__};
            eval "use $pkg";
            if ($@) {
...
# set up your @INC or -I options incorrectly.  For example, you used -I /path/to/modules/System/
...
# you'll get wierd failures if it loads this module instead of the right one.
{
    ...
    if ($caller_info[3] eq '(eval)' and $caller_info[6] eq 'Config.pm') {
        die "package System::Config was loaded from a 'use Config' statement, and is not want you wanted.  Are your \@INC and -I options correct?";
    }
}
...
System::Site - hostname oriented site-based configuration
