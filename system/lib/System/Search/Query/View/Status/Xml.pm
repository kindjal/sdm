package System::Search::Query::View::Status::Xml;

use strict;
use warnings;
use System;

class System::Search::Query::View::Status::Xml {
...
sub _generate_content {
    ...
    my $solrQuery = $query;
    my $response = System::Search->search(
        $solrQuery,
        $params
    );
...
#   END OF FACET XML GENERATION
    ...
    $results_node->addChild(
        System::Search->generate_pager_xml( $response->pager, $doc ) );
...
#    my @ordered_docs = sort_solr_docs( $response->docs );
    ...
    my @result_nodes =
      System::Search->generate_result_xml( \@ordered_docs, $doc, $format );
...
sub icon_prefix {
    ...
    my $icon_prefix = {
        'disk_group'         => 'system_disk_group',
        'disk_volume'        => 'system_disk_volume',
    };
...
sub sort_solr_docs {
    ...
    my @ordered_doc_classes = System::Search->searchable_classes();
