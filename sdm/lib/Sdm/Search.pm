...
#Don't "use Sdm;" here or we introduce a circular dependency.
use UR;
use Sdm::Memcache;
...
sub _delete_by_doc {
    ...
    my $memcached = Sdm::Memcache->server;
...
sub cache_key_for_doc {
    ...
    return 'sdm_search:' . $doc->value_for('id');
}
...
sub generate_result_xml {
    ...
    for my $doc (@docs) {
        ...
        require Sdm; #It is only at this point that we actually need to load other objects
        ...
        if($views{$object->class}) {
            ...
            if(my $view_class = $class->_resolve_result_xml_view($object_class)) {
                ...
                if($format eq 'xsl' or $format eq 'html') {
                    $view_args{xsl_root} = Sdm->base_dir . '/xsl';
                }
        ...
        if(defined $views{$object->class}) {
            ...
            $view->_update_view_from_subject();
             ...
             if(my $view_class = $class->_resolve_solr_xml_view($object)) {
                 ...
                 Carp::confess('To make an object searchable create an appropriate ::View::Solr::Xml that inherits from Sdm::View::Solr::Xml.');
             }
        }
...
my %commit_cb = ();
my %delete_cb = ();
#This should be called from Sdm.pm, so typically it won't need to be called elsewhere.
...
=head1 SYNOPSIS
  ...
  Sdm::Search->add(@objects);
  Sdm::Search->delete(@objects);
  Sdm::Search->is_indexable($object);

  Sdm::Search->search($query, $options);
...
To make a new type of object indexable by Solr, create a subclass of Sdm::View::Solr::Xml for
that object.  For example, to make a Sdm::Individual indexable, the class
Sdm::Individual::View::Solr::Xml was created.  The class should define the
