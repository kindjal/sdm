...
BEGIN {
    unless ($ENV{SDM_ASSET_DATABASE_DRIVER}) {
        $ENV{SDM_ASSET_DATABASE_DRIVER} = "SQLite";
    }
    unless ($ENV{SDM_ASSET_DATABASE_HOSTNAME}) {
        $ENV{SDM_ASSET_DATABASE_HOSTNAME} = "localhost";
    }
};

my $hostname = $ENV{SDM_ASSET_DATABASE_HOSTNAME};

if ($ENV{SDM_ASSET_DATABASE_DRIVER} eq "SQLite") {
...
} elsif ($ENV{SDM_ASSET_DATABASE_DRIVER} eq "Pg") {
    class Sdm::DataSource::Asset {
        is => [ 'UR::DataSource::Pg' ],
        has_constant => [
...
sub _sql_like_supports_escape {
    ...
    if ($ENV{SDM_ASSET_DATABASE_DRIVER} eq "Pg") {
        return 0;
    }
    return 1;
}
