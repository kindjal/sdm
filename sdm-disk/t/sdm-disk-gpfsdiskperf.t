...
ok( my $vol = SDM::Disk::Volume->create( @params ), "ok: create test volume");

# Test insufficient creation params
@params = ();
ok( ! defined SDM::Disk::GpfsDiskPerf->create( @params ), "ok: create fails on empty params");

@params = ( gpfsFileSDMPerfName => "/gscmnt/gc2111" );
ok( ! defined SDM::Disk::GpfsDiskPerf->create( @params ), "ok: create fails on insufficient params");
...
ok( defined SDM::Disk::GpfsDiskPerf->create( %params ), "ok: create works");
ok( defined UR::Context->commit, "ok: commit succeeds" );
$params{gpfsDiskPerfName} = 'ams2k6lun000f';
ok( defined SDM::Disk::GpfsDiskPerf->create( %params ), "ok: create works");

foreach my $ref ( SDM::Disk::GpfsDiskPerf->get() ) {
    $ref->delete();
